...
public class ITP_SubTime extends CommandBase {
    private String usage = "itp_subtime [player] ticks_to_subtract";
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if(args.length == 1)
        {
            if(sender instanceof EntityPlayerMP)
            {
                ...
                if(player.getPresenceTime() >= Integer.valueOf(args[0]))
            ...
            {
                IGhostPlayer player = entityPlayer.getCapability(GhostPlayerProvider.GHOST_PLAYER_CAPABILITY, null);
                if(player.getPresenceTime() >= Integer.valueOf(args[1]))
